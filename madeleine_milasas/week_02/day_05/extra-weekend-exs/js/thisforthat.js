...
const grammarCheck = function ( word ) {
  if (word[0].toUpperCase() === 'A' || word[0].toUpperCase() === 'E' || word[0].toUpperCase() === 'I' || word[0].toUpperCase() === 'O' || word[0].toUpperCase() === 'U') {
    return 'n';
  }
  return '';
};

const randomThis = function () {
  return _this[ Math.floor( Math.random() * _this.length ) ];
};

const randomThat = function () {
  return _that[ Math.floor( Math.random() * _that.length ) ];
};

const thisForThat = function () {
  const thisWord = randomThis();
  const n = grammarCheck( thisWord );
  console.log( `So.... your startup is basically like a${n} ${ thisWord } for ${ randomThat() }?` );
};
