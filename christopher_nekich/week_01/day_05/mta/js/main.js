...
// const planTrip = function(start, stop){
  // let startIndex = lLine.indexOf(start);
  // let stopIndex = lLine.indexOf(stop);
  //
...
const journeyA = function(startLine, start, stopLine, stop){ // total number of stops in trip that requires a transfer
  let journey = []
  let startIndexA = mta[startLine].indexOf(start);
  let stopIndexA = mta[startLine].indexOf('Union Square');
  stopsA = difference(startIndexA, stopIndexA);
...
const journeyB = function(stopLine, stop){
  ...
  if(startIndexB > stopIndexB){
    ...
    for (var i = startIndexB + 1; i <= stopIndexB; i++) {
      journey.push(mta[stopLine][i])
    }
  }
  return journey
